  ...
  return new Promise(function(resolve, reject) {
    ...
    img.onload = function() {
      try {
        ...
        var maxDim = 768; // Cân bằng: đủ rõ để đọc chính xác số, vẫn tiết kiệm hơn 1600px
        var scale = 1.0;
        ...
        // Grayscale nhẹ + contrast vừa phải: giữ chi tiết số, bỏ màu thừa
        var imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        var d = imgData.data;
        for (var i = 0; i < d.length; i += 4) {
          var gray = 0.299 * d[i] + 0.587 * d[i+1] + 0.114 * d[i+2];
          gray = Math.min(255, Math.max(0, (gray - 60) * 1.4));
          d[i] = d[i+1] = d[i+2] = gray;
        }
        ctx.putImageData(imgData, 0, 0);
        resolve({ primary: canvas.toDataURL('image/jpeg', 0.5) });
      } catch(e) { reject(e); }
    };
    img.onerror = reject;
    img.src = url;
